...
def CTCLoss(y_true, y_pred):
    # Compute the training-time loss valu
...
def get_text_output():
    audio_path = r'static/Recordings/recording_converted.wav'  # 'LJ038-0015.wav' #
    transcription = 'bla bla'

    def convert_audio(audio_segment, target_sample_rate, target_bit_depth, target_channels):
        # Convert the sampling rate
        audio_segment = audio_segment.set_frame_rate(target_sample_rate)

        # Convert the bit depth by exporting to a WAV file with the desired bit depth
        temp_file_path = "static/Recordings/temp_converted_audio.wav"
        audio_segment.export(temp_file_path, format="wav", parameters=["-b", str(target_bit_depth)])

        # Re-load the WAV file to ensure bit depth and other properties are set correctly
    ...
    audio_4 = AudioSegment.from_file(file="static/Recordings/recording.wav")
    # Convert Audio 1 to match Audio 2 format
    converted_audio = convert_audio(audio_4, target_sample_rate=22050, target_bit_depth=16, target_channels=1)

    # Export the converted audio to a new file
    converted_audio.export("static/Recordings/recording_converted.wav", format="wav")
